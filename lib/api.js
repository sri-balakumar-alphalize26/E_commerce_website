...
/* A request that never settles is worse than one that fails: the caller's
   "saving…" or "placing your order…" sits there for the rest of the session
   with nothing to react to. Odoo's own `limit_time_real` is 120s, so this is
   deliberately longer than any answer the shop will legitimately give. */
...
async function send(url, { method, body, raw }) {
  ...
  try {
    res = await fetch(url, {
      ...
      /* Older browsers have fetch but not AbortSignal.timeout; there the
         request simply behaves as it did before. */
